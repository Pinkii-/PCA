...
//	qsort(empleats, N, sizeof(Templeat), compare);
	...
	{
		ii = empleats_NID[i].i;
		memset(&empleat, 0, sizeof(Templeat));
		empleat.NID = empleats_NID[i].NID & 0xFFFFFFFF;
		empleat.Nom[0] = 'A' + ((ii>>3) + (ii<<2)) & 0x15;
		empleat.Cognoms[0] = 'a' + ((ii>>2) + (ii<<3)) & 0x15;
		empleat.horesMes[ii % MAX_DIAS_MES] = ii;
		empleat.Categoria[ii % MAX_CATEGORIA] = (ii << 2) * ii;
		empleat.ptrClauForaneaDepartament = ii<<10 + ii<<14 + ii>>3;
		empleat.ptrClauCategoria = ii<<24 + ii<< 18 + ii<<10 + ii>>2;
#ifdef EXTES     
        empleat.Address[0] = 'A' + ((ii>>4) + (ii<<2)) & 0x15;
        empleat.Nom_parella[0] = 'a' + ((ii>>5) + (ii<<2)) & 0x15;
  		empleat.Cognoms_parella[0] = 'A' + ((ii>>5) + (ii<<3)) & 0x15;
#endif
		write(1, &empleat, sizeof(Templeat));
	}
